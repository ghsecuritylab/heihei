...
void usart1_init(u32 bound)
{
  ...
  //USART 初始化设置
	...
	USART_InitStructure.USART_Mode = USART_Mode_Tx;	   //收发模式   这里可以配置仅发 或仅收
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		if((USART1_RX_STA&0x8000)==0)//接收未完成
		{
			if(USART1_RX_STA&0x4000)//接收到了0x0d
			{
				...
				else USART1_RX_STA|=0x8000;	//接收完成了 			  //接收到回车的后字节  置位状态寄存器 
			}
			else //还没收到0X0D
			{	
				if(Res==0x0d)USART1_RX_STA|=0x4000;					 //接收到回车的前一字节  置位状态寄存器
				else
				{
					USART1_RX_BUF[USART1_RX_STA&0X3FFF]=Res ;			//将接收的数据 存入数组中
					USART1_RX_STA++;									//长度+1 为下一次做准备
