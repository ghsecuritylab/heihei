...
void enc28j60WriteOp(unsigned char op, unsigned char address, unsigned char data)
{
	...
	SPI1_ReadWriteByte(dat);				  //SPI1 写
	dat = data;
	SPI1_ReadWriteByte(dat);				  //SPI1 写操作数据
...
void enc28j60ReadBuffer(unsigned int len, unsigned char* data)
{
	...
	while(len)
	{
        len--;
        // read data
        *data = (unsigned char)SPI1_ReadWriteByte(0);
        data++;
	}
	*data='\0';
	ENC28J60_CSH();
...
//enc28j60硬复位
void enc28j60HardwareReset(void)
{
	ENC28J60_RST = 0;
	delay_ms(200);
	ENC28J60_RST = 1;
...
//enc28j60的IO口初始化
void ENC28J60_GPIO_Init(void)
{
	GPIO_InitTypeDef  GPIO_InitStructure;

	RCC_APB2PeriphClockCmd( RCC_APB2Periph_GPIOA | RCC_APB2Periph_GPIOE, ENABLE );	

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;				//enc28j60的CS脚初始化
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		
	GPIO_Init(GPIOA, &GPIO_InitStructure);					 
	GPIO_SetBits(GPIOA, GPIO_Pin_4);			

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;				//enc28j60的RST脚初始化
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		
